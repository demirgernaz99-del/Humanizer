...
self.addEventListener('activate', function (e) {
  e.waitUntil(caches.keys().then(function (keys) {
    return Promise.all(keys.filter(function (k) { return k.indexOf('zugradar-') === 0 && k !== V; }).map(function (k) { return caches.delete(k); }));
...
self.addEventListener('fetch', function (e) {
  ...
  if (isPage) {
    ...
    e.respondWith(fetch(req).then(function (res) {
      ...
      return res;
    }).catch(function () { return caches.match(req).then(function (r) { return r || caches.match('zugradar.html'); }); }));
    return;
  }
