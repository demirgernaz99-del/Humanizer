...
Dieser Leitfaden führt dich vom jetzigen Stand bis zum ersten zahlenden Kunden.
Er ersetzt keine Rechts- oder Steuerberatung. Die markierten Punkte solltest du von Fachleuten prüfen lassen.
...
2. **Shop über einen „Merchant of Record“ anlegen.** Empfehlung: **Lemon Squeezy**. Der Anbieter verkauft
   im eigenen Namen, kassiert und führt die Mehrwertsteuer in allen EU-Ländern ab und stellt Rechnungen aus.
   Du bekommst eine Auszahlung. Das spart dir die EU-Umsatzsteuer-Bürokratie (OSS). Alternativen: Paddle, Gumroad.
   Die Gebühren ändern sich; prüfe sie vor dem Start auf der Anbieterseite.
3. **Produkt anlegen:** „Zugradar Pro“ mit drei Varianten (monatlich, jährlich, einmalig).
   Bei jeder Variante **License keys** einschalten, Aktivierungslimit z. B. 3 Geräte.
4. **`seller.json` ausfüllen:**
   - `checkoutUrl`: der Checkout-Link aus dem Shop
   - `license.storeId` und `license.productIds`: aus dem Shop-Dashboard. Dann akzeptiert die App nur deine Schlüssel.
...
5. **Lizenz testen:** Im Testmodus des Shops einen Kauf auslösen und den Schlüssel in der App unter
   „Pro holen → Lizenzschlüssel eingeben“ aktivieren. Meldet der Browser einen CORS-Fehler, den kleinen Proxy
   `server/license-proxy.js` als Cloudflare Worker deployen und dessen Adresse in `src/config.js` (`license.api`) eintragen.
...
- **Lizenz-API im Browser:** Ob Lemon Squeezy direkte Browser-Anfragen erlaubt (CORS), musst du mit einem Testkauf
  prüfen. Für den Fall, dass nicht, liegt der Proxy fertig bereit.
